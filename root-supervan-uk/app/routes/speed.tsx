...
import type { Route } from "./+types/speed";
...
const SpeedGauge = ({ value, max, label, active }: { value: number, max: number, label: string, active: boolean }) => {
...
export function meta({ }: Route.MetaArgs) {
    return [
        { title: "Internet Speed Test | SuperVan Digital" },
        { name: "description", content: "Test your internet connection speed with our professional, lightweight tool." },
    ];
}
...
export default function Speed() {
    ...
    const updateResults = (updates: any) => {
    ...
    const formatDate = (dateString: string) => new Date(dateString).toLocaleDateString() + ' ' + new Date(dateString).toLocaleTimeString();

    const copyToClipboard = async (text: string, id: number | string = 'share') => {
        try {
